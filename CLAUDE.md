...
**Current test files (622 tests, 24 files):**
...
- `storage.test.js` — `loadData`, `saveData`, `importJSON` (now resolves `{ data, skipped }`), `importBudget`, `buildCSVString`, `hasStoredData`, `withDefaults` (an `actualIncome` year surviving a round trip and a wrong-typed one rejected, expense-date repair, amount coercion on expenses and recurrings, absent-field passthrough, a fund keeping its `kind` and `contributions`), `validateImportData` sanitising
- `dataTransforms.test.js` — `varianceStatus` (a spending row under plan is `onTrack` not a win, a savings row under plan is `behind`, a missing side is `unset`, a confirmed `0` still grades), `monthlyCategoryTotals` (one-pass bucketing, other years excluded, `'2026-02-31'` staying in the month it names, malformed rows skipped rather than filed under NaN), `fundActuals` (a savings fund reads `contributions` and ignores expenses; a spending fund with nothing mapped is all-`null`, not all-zero), `unmappedSpend` (a category mapped to two funds subtracted **once**), `statusKey` (incl. exactly-on-the-allocation being `warn`, not `over`), `elapsedMonths` / `isSavingsFund` (only `'savings'` counts; absent means spending) / `goalProgress` (`saved` is the confirmed sum and ignores the plan, a filled-in plan with nothing confirmed reads as zero, `expected` reproduces the old elapsed-plan number, a confirmed future month still counts, a confirmed `0` is a confirmation while an unconfirmed month is not, `null` months add to no total, cap at 100%, zero target isn't Infinity, `NO_PROGRESS` carries every returned key), `generateRecurringExpenses` (incl. `skippedMonths`, day clamping, and the `recurringId|YYYY-MM` existence index: per-template isolation, month padding, hand-entered rows, dateless rows), `applyBudgetCopy` (incl. custom income rows: absent list copies to `[]`, names carry over with fresh ids and empty amounts; a savings fund keeps its `kind` but not last year's `contributions`; a spending fund gains no `kind` key), `isEmptyData`, `applyExpenseDeletion` (incl. the delete→regenerate round trip)
...
- `LiveOverview.test.jsx` — the clock is pinned to August 2026 with `vi.setSystemTime`, so "how much of the year has happened" is a fact and not the runner's calendar. Covers: the empty state routing to Budžet; a spending row summing its mapped categories; **over plan flags while under plan reads `onTrack`, not as a win**; a fund with nothing mapped reading `—` plus `nema kategorija`; a savings row reading `contributions` and ignoring a stale category mapping, with the **inverted** scale (under = behind, over = ahead) and a confirmed `0` distinguished from an unconfirmed month; `Van budžeta` catching untracked spend, not double-subtracting a category mapped to two funds, and still reporting spending whose only mapping is to a savings fund; income showing the plan until overridden, an override flagging the variance while untouched months still read off the plan, typing calling `setActualIncome`, **emptying the field sending `null`** (the revert), and a custom income row saving under its own id; future months rendering plan-only and never graded; the `Ukupno` column comparing against the plan **through the elapsed months only** (8 × 500, not 12 × 500)
...
```js
{
  ...
  actualIncome: { [year]: { [rowKey]: [12 values] } },
...
`actualIncome` is what really came in, as a **sparse override on top of the budgeted plan** — see the live-overview section below. `rowKey` is `'plata'`, `'bonus'`, or a custom income row's id; `null` in a slot means "no override, show the plan".
...
`setActualIncome` is deliberately **not** in this table: clearing an actual-income cell reverts the month to the budgeted plan rather than destroying a record, so there is nothing to undo.
...
### Plan, actual, and the live overview

A tester used **Budžet** as a live tracker: they entered the year's plan, then rewrote it as reality arrived — raising a category after overspending, lowering it after underspending, bumping income after a raise. That destroys the benchmark. Once the plan says what happened, the variance ("I was 200 € wrong about food in May") no longer exists and there is nothing left to measure against.

It was a UX signal, not user error. They were asking a question the app had no page for: *"It's August. What do my finances actually look like now, against what I planned?"* The budget grid was the closest thing available, so they repurposed it. The app had two layers and needed three:

```
Budžet          →  Live godišnji pregled  →  MonthView
(the plan)         (derived output)          (monthly activity)
input                                        input
```

**`LiveOverview.jsx` is derived output.** It mirrors the budget grid's shape so the layout is already familiar, but with one deliberate exception (income, below) it computes everything and writes nothing. Keeping it read-only is what stops it becoming a second budget page and reintroducing the original problem.

**Under plan is not universally good, so there is no single red/green scale.** `varianceStatus(actual, planned, { higherIsBetter })` in `dataTransforms.js` is the one place that decides:

| Row | `higherIsBetter` | Under plan reads as |
|---|---|---|
| spending fund | `false` | `onTrack` — *neutral*, never green. The vacation may simply not have happened yet |
| savings fund | `true` | `behind` — a real shortfall |
| income | `true` | `behind` |

A fund already knows which scale it is on via `isSavingsFund`. Colouring a spending row green for underspending was the specific mistake this table exists to prevent.

**Each row's "actual" comes from a different place, and the row decides which:**
- **spending fund** → expenses in its `trackingMaps` categories (`fundActuals`). A fund with nothing mapped returns all-`null`, not all-zero — "not tracked" and "tracked, spent nothing" are different answers, and the row says `nema kategorija` rather than reporting a fake zero.
- **savings fund** → `contributions`, the confirmations recorded in `SavingsPanel`. Reading expenses here would measure it against spending it has nothing to do with, including through a stale mapping left over from before 💰 was switched on.
- **Van budžeta** → `unmappedSpend`, everything spent on a category no *spending* fund maps. Without this row the grid quietly under-reports the year. Two things it gets right and a naive version doesn't: it subtracts over the **union** of mapped categories (a category mapped to two funds is deliberately counted in both fund rows, but subtracting it twice would push this row negative), and `LiveOverview` passes it **only the spending funds' maps, not the whole `trackingMaps` year**. A savings fund can still carry a stale mapping from before its 💰 flag was set, and its row shows confirmations rather than expenses — so counting those categories as tracked would make that spending vanish from the page entirely, visible in no row at all.

**Income is the one editable thing on the page**, because nothing in the app records income as a transaction — it exists only as a budget plan. It is stored in `actualIncome`, *never* written back into `budget[year].income`, and it is an **override**, not a plain value: an untouched cell shows the plan, typing replaces it for that month, and **emptying the cell reverts to the plan**. That last part is free — `parseAmountInput` already returns `null` for a cleared field, which is exactly the "no override" value. `actualIncome/set` is silent and is deliberately *not* routed through `deleteWithUndo`: clearing a cell restores the plan rather than destroying a record.

**A month that hasn't happened is not a month where nothing was spent.** Columns at index `>= elapsedMonths(year)` render plan-only and are never graded, or every remaining month of the year would read as a saving and every future savings row as a shortfall. For the same reason the `Ukupno` column is **year-to-date** — actual so far against the plan *through the elapsed months only*. Comparing eight months of actuals against twelve months of plan reports every row in the app as comfortably under budget in August.

Forecast — a third number beside plan and actual — is deliberately out of scope and written up in `BACKLOG.md`. The tester editing *future* months after a raise was really trying to build one.
...
- **`Header.jsx`** — nav is split into two groups by thin `.header__sep` dividers: primary (Početna, Prethodne, 🔍) and tools (Budžet, Live pregled, Kategorije), followed by autosave status and theme toggle. "Live pregled" is the nav label for `LiveOverview`, whose page title is the fuller "Live godišnji pregled" — the full name wraps in the sidebar. "Prethodne" is active for both `view === 'previous'` and `view === 'month'`. There is no separate Praćenje nav item — tracking is inline in BudgetView.
...
- **`LiveOverview.jsx`** — the plan-vs-actual year grid (see the section above for *why* it works the way it does). Local `year` state with the same `‹ YEAR ›` nav as BudgetView. It **reuses the `.bg` table wholesale** from `budget-view.css` — `.bg__th`, `.bg__label-col`, `.bg__col--current`, `.bg__cell`, `.bg__subtotal-row`, `.bg__balance-row`, `.bgc` — which is also what buys it dark mode for free, since `dark.css` already overrides those. `live-overview.css` adds only what is genuinely new (the two-line cell, the directional status colours, the month-header buttons) and uses `var(--…)` exclusively, so `dark.css` needs no counterpart rules. Keep `colSpan={14}` for full-width rows. Every cell is two lines: actual over `/ plan`. **Month column headers are `<button>`s** that `navigateTo('month', year, m)` — the drill-down sits on the header rather than the cells precisely so it doesn't fight the editable income cells. Rows carry `data-testid={`lgo-row-${label}`}` for `within()`-style scoping, since grid cells have no role to query by. One `useMemo` buckets the year's expenses via `monthlyCategoryTotals`; doing it per cell with `getExpensesForMonth` would walk the whole expense array 12 × funds times.
- **`BudgetCell.jsx`** — the click-to-edit amount cell, extracted from `BudgetView` so `LiveOverview` shares it rather than copying it; two copies would drift and then `1.500` would parse one way in one grid and another way in the other. Commit on blur/Enter, abandon on Escape, and `parseAmountInput`'s three-way `number | null | undefined` result drives all of it. `children` overrides the display (that is how LiveOverview renders actual-over-plan in the same shell) while the draft still seeds from `value`, so clicking an inherited income cell opens an *empty* field — committing it empty writes `null` and it stays inherited rather than silently pinning itself to the plan. It is `role="button" tabIndex={0}` with Enter/Space, so both grids are keyboard-operable.
...
It also holds the live-overview arithmetic — `varianceStatus`, `monthlyCategoryTotals`, `fundActuals`, `unmappedSpend` — plus `statusKey` / `THRESHOLD_WARN`, which moved out of `BudgetPanel.jsx` when the overview started grading the same spent/allocated ratio. Two copies would drift into disagreeing about the same month. **Don't reach for `filterByCategories` when intersecting a fund with its mapped categories**: an empty selection there means "no filter, return everything", which is the exact opposite of what a fund with nothing mapped needs — that is why `fundActuals` does its own guard.
...
Styles live in **`src/styles/`, one file per component or shared concern**, stitched together by `src/index.css`. `index.css` holds no rules — only a table of contents and an ordered list of `@import`s — and stays the single entry point (`main.jsx` imports it). BEM-ish class names per component (`.budget__*`, `.bg__*`, `.lgo*`, `.cat-*`, `.bp-*`, `.sp-*`, `.gsearch__*`, `.goal-*`, etc.). `live-overview.css` is the one file that deliberately extends another component's classes rather than standing alone — it layers `.lgo*` onto `budget-view.css`'s `.bg` grid, which is what keeps the two grids visually identical and inherits the existing dark-mode overrides. Always use `var(--text)`, `var(--bg-card)`, etc. on new inputs/elements so they respect the theme automatically.
